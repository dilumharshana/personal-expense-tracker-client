...
const MasterDataPage: React.FC = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const isTypeExists = masterData.find(
        (title: MasterData) =>
    ...
    <Box>
      ...
      {deleteMutation.isError && (
        <Alert severity="error" sx={{ mb: 2 }}>
          {((): string => {
            const axiosError = deleteMutation.error as AxiosError<{
              message?: string;
              error?: string;
            }>;
            return (
      ...
      <TableComponent
        isLoading={isLoading}
        onEdit={(row: unknown) => handleEdit(row as MasterData)}
        onDelete={handleDelete}
        isDeletePending={deleteMutation.isPending}
        columns={ExpenseTypesTableColumns}
        tableRows={
          <>
            {isLoading ? (
              <TableRow>
