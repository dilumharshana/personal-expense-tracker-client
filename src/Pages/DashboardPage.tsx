...
const Dashboard: React.FC = () => {
  ...
  const expensePatterns = React.useMemo(() => {
    ...
    const typeAmounts = dashboardData?.dashBoardData?.reduce(
      (acc, expense) => {
        const typeTitle = expense?.type
          ? getMasterDataTitle(expense?.type)
          : "Uncategorized";
  ...
  const dailyExpenses = React.useMemo(() => {
    ...
    const dailyAmounts = dashboardData.dashBoardData.reduce(
      (acc, expense) => {
